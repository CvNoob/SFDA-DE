...
class TargetCategoricalDataset(Dataset):

    def __init__(self):
        super(TargetCategoricalDataset, self).__init__()

    def initialize(self, target_paths,
                   classnames, class_set,
                   target_batch_size, seed=None,
                   transform=None, **kwargs):
        ...
        self.seed = seed
        self.classnames = classnames
        self.batch_sizes = {'target': {}}
        cid = 0
        for c in self.class_set:
            self.batch_sizes['target'][cid] = min(target_batch_size, len(self.data_paths['target'][cid]))
            cid += 1
    ...
    def name(self):
        return 'TargetCategoricalDataset'
